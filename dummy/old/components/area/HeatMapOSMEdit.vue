<template>
  <div class="grid">
    <div class="col-9">
      links TODO https://dev.to/camptocamp-geo/integrating-an-openlayers-map-in-vue-js-a-step-by-step-guide-2n1p
...
export default {
  ...
  watch: {
    selectedAreas: {
      handler: async function (newValue) {
        this.$emit("select", { areas: newValue });
      },
      deep: true,
    },
    heatmap: function () {
  ...
  methods: {
    ...
    scaleImage(stage, bgImage) {
      if (bgImage != null) {
        var scale = Math.min(this.width / bgImage.width, this.heightł / bgImage.height);
        this.scale = scale;
        console.info(scale);
