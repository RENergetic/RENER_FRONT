  ...
  <Dialog v-model:visible="dialog" :style="{ width: '75vw' }" :maximizable="true" :modal="true" :dismissable-mask="true">
    <div class="ren">
      ...
      <div v-for="(detail, index) in detailsList" :key="index" class="property-form">
        <InputText v-model="detailsList[index].key" placeholder="Key" @keyup="checkForDuplicatesAndNewLineNeed()" />
        <InputText v-model="detailsList[index].value" placeholder="Value" />
        <div v-if="detail.error !== null" class="property-form-error">
          <span v-if="detail.error === 'dup'">Duplicated</span>
        </div>
        <!-- <span>{{ detailsKey }}</span>
        <InputText v-model="details[detailsKey]" type="text"></InputText> -->
      </div>
  ...
  methods: {
    open(selectedAsset, detailsKeys) {
      ...
      for (const [key, value] of Object.entries(selectedAsset.details)) {
        this.detailsList.push({ index: this.detailIndex, key: key, value: value, error: null });
        this.detailIndex += 1;
      }
      this.detailsList.push({ index: this.detailIndex, key: "", value: "", error: null });
      this.detailIndex += 1;
    ...
    checkForDuplicates() {
      ...
      this.detailsList.forEach((itemA) =>
    ...
    checkForDuplicatesAndRemoveEmpty() {
      ...
      this.detailsList = this.detailsList.filter((item) => item.key !== null && item.key.length > 0);
