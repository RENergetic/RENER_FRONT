<template>
    ...
    <TabView ref="signTable" v-model:activeIndex="activeTab">
        <TabPanel header="Your User Data">
            <div class="p-fluid grid">
                <span class="p-float-label p-mt-4">
                    <InputText id="username" v-model="user.username" type="text" :class="error.username.value?'p-invalid':''" @focus="error.username.value = false" />
                    <label for="username">Choose a Username</label>
                </span>
                <small id="username-help" :class="error.username.value?'p-error':'p-sr-only'">{{error.username.msg}}</small>

                <span class="p-float-label p-mt-4">
                    <InputText id="firstname" v-model="user.firstName" type="text" :class="error.firstName.value?'p-invalid':''" @focus="error.firstName.value = false" />
                    <label for="firstname">Your First Name</label>
                </span>
                <small id="firstname-help" :class="error.firstName.value?'p-error':'p-sr-only'">First name is required</small>

                <span class="p-float-label p-mt-4">
                    <InputText id="lastname" v-model="user.lastName" type="text" :class="error.lastName.value?'p-invalid':''" @focus="error.lastName.value = false" />
                ...
                <span class="p-float-label p-mt-4">
                    <Password id="password" v-model="user.credentials[0].value" :class="error.password.value?'p-invalid':''" @focus="error.password.value = false" />
        ...
        <TabPanel header="Your Home">
            <div class="p-fluid grid">
                <span class="p-float-label p-mt-4">
                    <InputText id="name" v-model="asset.name" type="text" :class="error.homeName.value?'p-invalid':''" @focus="error.homeName.value = false" />
                ...
                <span class="p-float-label p-mt-4">
                    <InputText id="geo_location" v-model="asset.geoLocation" type="text" :class="error.geoLocation.value?'p-invalid':''" @focus="error.geoLocation.value = false" />
                ...
                <span class="p-float-label p-mt-4">
                    <Dropdown id="type" v-model="selectedType" :options="types" optionLabel="text" :class="error.type.value?'p-invalid':''" @focus="error.type.value = false" />
    ...
    <Toolbar>
        ...
        <template #end>
            <Button label="Sign in" icon="pi pi-user-plus" class="p-button-sucess" style="width:10vw" @click="signIn()" />
        </template>
    </Toolbar>
    </div>
</template>
    ...
    methods: {
        signIn(){
            ...
            if (!this.user.firstName)
                this.error.firstName.value = true;
            else this.error.firstName.value = false;
            if (!this.user.lastName)
                this.error.lastName.value = true;
            ...
            if (!this.user.credentials[0].value)
            ...
            if (!error){
                ...
                this.$ren.userApi.register(this.user).then(()=> {
