...
<template>
	<div>
        <DataTable v-model:expandedRows="users.roles" :value="users" dataKey="id" responsiveLayout="scroll">
            ...
            <template #expansion="user">
                <div v-if="user.data.roles != undefined && user.data.roles.length > 0" class="orders-subtable" style="margin:0 2rem">
                    <DataTable :value="user.data.roles" responsiveLayout="scroll">
                        <Column :exportable="false" style="min-width:8rem">
                            <template #body="role">
                                <Button icon="pi pi-trash" class="p-button-rounded p-button-danger" @click="openDialog('deleteRole', user.data, role.data)" />
                            </template>
                        </Column>
                    ...
                    <div class="col-12 md:col-4">
                        <div class="p-inputgroup">
                            <select style="width:100%; text-align:center" @change="selectRole($event, user.data)">
                                <option v-for="role of availableRoles(user.data)" :key="role.id" :value="JSON.stringify(role)">{{role.name}}</option>
                            </select>
        ...
        <Dialog v-model:visible="deleteDialog.visible" :style="{width: '450px'}" :header="deleteDialog.header" :modal="true">
            ...
            <template #footer>
                <Button label="No" icon="pi pi-times" class="p-button-text" style="color:var(--orange-300)" @click="deleteDialog.visible = false"/>
